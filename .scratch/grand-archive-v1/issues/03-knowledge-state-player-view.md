...
- [ ] Player View 不暴露 Card Instance、Object 或其他引擎內部 ID，也不洩漏對手手牌或牌庫順序。
- [ ] 合法行動與待決選擇只包含該玩家當下可見且可選的 handle。
- [ ] 過期、跨玩家、偽造或已撤銷 handle 均被拒絕且不改變正式狀態。
- [ ] 洗牌或失去追蹤權會撤銷舊 handle；已公開事件仍可回顧但不能重新連結該實體。
