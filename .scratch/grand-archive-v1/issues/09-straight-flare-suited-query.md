...
- [x] 查詢以 Card Face 的 printed 資料計算不同 reserve cost，不由單卡 handler 自行掃描或推測。
- [x] Player View 只列出合法目標，宣告與結算分別重驗目標合法性。
- [x] 所有必要目標失效時完整 fizzle；仍有合法必要目標時只對合法目標結算。
- [x] 正常、零種 cost、重複 cost 與目標失效案例均有規則來源及 replay/hash 驗證。
