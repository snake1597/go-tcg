...
- [x] Smoke Bombs 只能選擇合法 Ally，banish 自身後授予本回合 stealth 並抽牌。
- [x] 攻擊目標在 stealth、taunt 與 true sight 改變後會透過共用規則重新驗證。
- [x] Trump Set 只能把 active attack 改指向不同且合格的 Suited Ally，並套用正確 duration 的 power/life。
- [x] 失去原目標、沒有替代目標、來源失效及 phase 結束案例均有確定性測試。
