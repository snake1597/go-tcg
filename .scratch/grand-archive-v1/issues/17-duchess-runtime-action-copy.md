...
- [x] 合法候選精確涵蓋牌組內所有符合 element、type 與 reserve cost 的 action。
- [x] Copy 沿用來源 Card Face 的 resolution behavior，但具有獨立 Object／Stack Item 身分與一次性追蹤。
- [x] 玩家可選擇不啟動；選擇啟動時仍完成 mode、target 與合法性驗證但不支付被免除的 activation cost。
- [x] 來源離開墓地、copy 目標失效及結算完成後的生命週期均有 replay/hash 回歸案例。
