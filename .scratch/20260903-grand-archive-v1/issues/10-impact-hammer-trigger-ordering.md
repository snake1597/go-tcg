...
- [x] 每事件觸發與每批次聚合觸發保留正確 batch、cause、parent flow 與 simultaneous metadata。
- [x] 單一合法順序自動推進；同玩家多個 triggers 時建立 Pending Choice 並等待其排序。
- [x] mode 與必要 target 在入 Stack 時固定，depending-on 類結果留到結算時計算。
- [x] Impact Hammer 的 On Wield 自傷即使來源之後離場仍依 Source Ref 與 LKI 正確結算。
