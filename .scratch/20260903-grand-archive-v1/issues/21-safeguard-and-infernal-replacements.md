...
- [x] Safeguard Amulet 的自我 banish 建立限定 Champion、非戰鬥傷害及 duration 的 delayed prevention。
- [x] Infernal Vessel 每次套用後重新計算 recover amount，並正確處理零或負結果及 recover cost 的差異。
- [x] 多個適用 replacement 由受影響內容的控制玩家排序；宣告中與已提交流程使用各自正確的 choice scope。
- [x] 原始意圖、每次替代與最終 committed events 保留完整 cause chain 並可重播。
