...
- [x] 建立玩家、Card Instance、Champion Object、起始 zones、Knowledge State、scheduler frame 與 revision。
- [x] 洗牌只使用版本化 PRNG，且 Spirit of Fire 的 On Enter 依規則順序各抽七張牌。
- [x] 抽牌以具有批次、原因及確定順序的 committed events 表示並正確投影給各玩家。
- [x] 開局抽牌遇到空牌庫時依法結束單局；相同 seed 的 setup 與 hash 可重播。
