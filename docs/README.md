...
| 想知道什麼 | 優先讀取 | 必要時接著讀 |
| --- | --- | --- |
| 固定牌組、卡面版本或建局資料流程 | [`card.md`](./card.md) | 對應 `card/*.json`、[`../card-data-manifest.json`](../card-data-manifest.json) |
| 某張卡依賴哪些機制或其他卡牌 | 對應的 Game Module 實作與測試 | [`rules-issues.md`](./rules-issues.md)、對應 `rules/` 文件 |
| 現在應該實作什麼 | [Grand Archive v1 工作追蹤](../.scratch/grand-archive-v1/issues/README.md#目前可執行) | [`implementation-features.md`](./implementation-features.md) |
...
| 文件 | 唯一負責內容 | 可以引用 | 不應重複 |
| --- | --- | --- | --- |
| [`card.md`](./card.md) | 卡面資料流程、固定牌組與最小建局驗證 | `card/*.json`、data manifest | 開發里程碑、架構原則、完整 ruling 內容 |
...
| [`testing.md`](./testing.md) | 測試層級、情境格式、品質 gate 與禁止模式 | development plan、rules | 功能 backlog、卡牌載入實作 |
...
關聯方向保持單向：Vue 讀取 PlayerView、Declaration 與 ConnectRPC 合約；ConnectRPC 讀取 PlayerView 與 Declaration，不反向定義規則；工作追蹤讀取規格，不把驗收內容複製回規格。`card.md` 是卡面資料與固定建局流程的唯一入口。下游文件不得反向複製上游事實。
...
| [0002](./adr/0002-reject-unsupported-card-mechanics-before-play.md) | 已由固定牌組的最小建局流程取代的 Support Set gate |
